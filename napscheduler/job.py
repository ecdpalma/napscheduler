...
from napscheduler.util import to_unicode, ref_to_obj, get_callable_name, \
    obj_to_ref
from napscheduler import triggers
...
class Job(Observable):
    ...
    def set_interval(self, weeks=0, days=0, hours=0, minutes=0, seconds=0):
        interval = timedelta(weeks=weeks, days=days, hours=hours,
                             minutes=minutes, seconds=seconds)

        last_run_time = self.next_run_time - self.trigger.interval

        if isinstance(self.trigger, triggers.IntervalTrigger):
            self.trigger.set_interval(interval)
            self.trigger.start_date = last_run_time
            self.compute_next_run_time(datetime.now())
