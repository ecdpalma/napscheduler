...
class TestDSTTimeDifference(object):
    ...
    def test_time_difference_daylight_1(self):
        earlier = datetime(2011, 3, 27, 2)
        later = datetime(2011, 3, 27, 4)
        eq_(time_difference(later, earlier), 3600)

    def test_time_difference_daylight_2(self):
        earlier = datetime(2011, 10, 30, 2)
        later = datetime(2011, 10, 30, 5)
...
def test_to_unicode():
    if sys.version_info[0] < 3:
        eq_(to_unicode('aa̦̦bb'), unicode('aabb'))
        eq_(to_unicode(unicode('gfkj')), unicode('gfkj'))
    else:
        eq_(to_unicode('aa̦̦bb'.encode('utf-8')), 'aabb')
